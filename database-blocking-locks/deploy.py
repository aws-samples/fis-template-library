#!/usr/bin/env python3
"""
Deploy Database Blocking Locks FIS Experiment
...
class ExperimentDeployer:
    def __init__(self, region, account_id):
        ...
        # Resource names
        self.fis_role_name = 'DatabaseBlockingLocks-FIS-Role'
        self.ssm_role_name = 'DatabaseBlockingLocks-SSM-Automation-Role'
        self.instance_profile_name = 'SSM-Managed-Instance-Profile'
        self.instance_role_name = 'SSM-Managed-Instance-Profile-Role'
        self.ssm_document_name = 'DatabaseBlockingLocks-Automation'
    ...
    def create_or_update_ssm_document(self):
        ...
        content = self.load_yaml_file('database-blocking-locks-automation.yaml')
    ...
    def create_or_update_fis_template(self):
        ...
        template = self.load_json_file('database-blocking-locks-template.json')
    ...
    def delete(self):
        ...
        print("\n" + "="*60)
        print("Database Blocking Locks - Delete Resources")
        print("="*60)
        ...
        try:
            ...
            template_id = None
            for tmpl in response.get('experimentTemplates', []):
                if tmpl.get('tags', {}).get('Name') == 'database-blocking-locks':
                    template_id = tmpl['id']
                    break
            if template_id:
    ...
    def deploy(self):
        ...
        print("\n" + "="*60)
        print("Database Blocking Locks - Deployment")
        print("="*60)
        print(f"Region: {self.region}")
        print(f"Account: {self.account_id}")
        print("="*60)
        ...
        try:
            ...
            self.create_or_update_role(
                ...
                'database-blocking-locks-fis-role-iam-policy.json',
                'Role for FIS to execute database blocking locks experiments'
            )

            self.create_or_update_role(
                self.ssm_role_name,
                'ssm-iam-trust-relationship.json',
                'database-blocking-locks-ssm-automation-role-iam-policy.json',
...
def main():
    parser = argparse.ArgumentParser(
        description='Deploy Database Blocking Locks FIS Experiment',
